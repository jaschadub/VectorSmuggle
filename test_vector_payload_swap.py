...
"""
Vector-Payload Dissociation Test Script

This script demonstrates the Vector-Payload Dissociation technique, a core steganographic
method where sensitive data is hidden in vector embeddings while benign content is stored
as the visible payload. This creates a mismatch where Qdrant dashboard shows innocent
content but the vectors contain hidden sensitive information.

The technique works by:
1. Creating steganographic embeddings from sensitive content using EmbeddingObfuscator
2. Generating benign decoy content using DecoyGenerator
3. Storing the sensitive vector with the benign payload in Qdrant
4. Demonstrating that only the benign content is visible in Qdrant dashboard
5. Proving that sensitive data can be recovered using proper tools

Usage:
    python test_vector_payload_swap.py [--collection-name NAME] [--output-dir DIR]
"""
...
class VectorPayloadSwapTester:
    """Test Vector-Payload Dissociation technique."""
    ...
    def __init__(self, config, logger: logging.Logger, collection_name: str = None):
        ...
        self.config = config
        self.logger = logger
        self.collection_name = collection_name or f"vector_payload_swap_test_{int(time.time())}"
...
P.S. Don't forget to bring your appetites! 😊"""
    ...
    def perform_vector_payload_swap(self) -> dict[str, Any]:
        ...
        self.logger.info(f"Storing {len(qdrant_points_for_storage)} vector-payload pairs in Qdrant")
        ...
        try:
            ...
            self.logger.info("✅ Successfully stored vector-payload swaps in Qdrant")
        except Exception as e:
            self.logger.error(f"Failed to store points in Qdrant: {e}")
            raise
    ...
    def generate_report(self, swap_results: dict[str, Any], hiding_demo: dict[str, Any],
        ...
        report = {
            ...
            "technical_details": {
                ...
                "storage_backend": "qdrant_vector_database"
            }
        }
    ...
    def run_test(self, keep_collection: bool = False) -> dict[str, Any]:
        ...
        try:
            self.logger.info("🚀 Starting Vector-Payload Dissociation test")
...
def setup_logging() -> logging.Logger:
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('vector_payload_swap_test.log')
        ]
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Vector-Payload Dissociation technique")
    parser.add_argument(
        "--collection-name",
    ...
    parser.add_argument(
        "--output-dir",
        default="results/vector_payload_swap",
    ...
    try:
        ...
        # Create tester
        tester = VectorPayloadSwapTester(config, logger, args.collection_name)
        ...
        results_file = output_path / f"vector_payload_swap_results_{timestamp}.json"

        with open(results_file, 'w') as f:
            json.dump(results, f, indent=2)

        # Generate summary report
        summary_file = output_path / f"vector_payload_swap_summary_{timestamp}.md"
...
def generate_markdown_summary(results: dict[str, Any]) -> str:
    ...
    return f"""# Vector-Payload Dissociation Test Summary
...
def print_test_summary(results: dict[str, Any]) -> None:
    ...
    print("\n" + "="*70)
    print("🎯 VECTOR-PAYLOAD DISSOCIATION TEST SUMMARY")
    print("="*70)
